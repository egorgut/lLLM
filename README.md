...
- Скачанная модель (по умолчанию `qwen3:8b`; для профиля `mid` — `qwen3:14b`,
  для `deep` — `qwen3:32b`, для `next` — `qwen3.8:27b`)
...
вместе с её дедлайнами (`--profile fast|mid|deep|next`). По умолчанию всё
...
поколения с измеренными дедлайнами. Сравнительный живой прогон evals по
профилям пока не сделан — см. журналы SPEC-017, PATCH-017-01 и PATCH-017-02.
