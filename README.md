...
| `tools/`          | Контракты и реестр инструментов (`ToolSpec`, `ToolRegistry`) |
...
Инструменты пока только **описываются**, но не исполняются: `tools/` содержит
`ToolSpec` (контракт инструмента: имя, описание, схемы входа и выхода) и
`ToolRegistry` (единый реестр — регистрация с валидацией, поиск по имени,
перечисление в порядке регистрации и генерация деклараций в формате function-tool
для Ollama). Исполнение инструментов, передача их модели и agent-loop появятся в
следующих шагах.
