...
инструменты; запрос сверх лимита **не исполняется** — это не изменилось. Изменилось
(SPEC-021) то, что происходит дальше: вместо обрыва хода делается **один финальный
запрос к модели без объявленных инструментов**, и модель отвечает тем, что уже
собрала, честно называя, что не успела. Ход завершается как `completed/budget_exhausted`,
ответ сохраняется. Рекурсия здесь исключена структурно, а не инструкцией: в этом
запросе инструментов просто нет. Каждый вызов показан в CLI как `[tool N/4]`. Структурированная
...
`final_answer`, `budget_exhausted`, `repeated_tool_call`, `model_timeout`,
`tool_timeout`, `turn_timeout`, `user_interrupt`). Успешных исходов два —
`completed/final_answer` и `completed/budget_exhausted` (ответ есть, но он оборван
исчерпанным бюджетом; отдельная причина нужна, чтобы этот факт не растворился в
обычном ответе). Любой другой исход не сохраняет частичный ответ, а
...
бы общий лимит `MAX_TOOL_CALLS_PER_TURN`. Это по-прежнему **терминальный** исход:
градация до финального ответа (SPEC-021) относится только к исчерпанному бюджету,
а не к зациклившейся модели.
...
- **Два независимых бюджета.** Активация **не** тратит рабочий вызов из
  `MAX_TOOL_CALLS_PER_TURN` (SPEC-021 §4.4: это оркестрация, а не работа —
  иначе ёмкость хода зависела бы от того, насколько удачно угадал роутер). Она
  считается против `MAX_SKILL_ACTIVATIONS_PER_TURN` (по умолчанию 2; выбор
  роутера в этот счёт не входит), а *попытки* активации ограничены собственным
  бюджетом цикла — `MAX_SKILL_ACTIVATIONS_PER_TURN + 1`. Плюс один здесь
  обязателен: отказанная попытка не увеличивает счётчик активаций, поэтому без
  него восстановимый ответ `activation_limit` был бы недостижим.
...
# Смена навыка по ходу (SPEC-018). Отдельный бюджет; рабочих вызовов активация
# не тратит (SPEC-021). Попытки ограничены циклом как это значение + 1.
# Выбор роутера в этот счёт не входит.
