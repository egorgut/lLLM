...
участка показывается однострочный индикатор `⠋ Working...`; он гасится и
...
вместе с её дедлайнами (`--profile fast|mid|deep`). По умолчанию всё работает
ровно как раньше, на `qwen3:8b`; `deep` поднимает `qwen3:32b` с бюджетами под
его латентность, `mid` (PATCH-017-01) закрывает разрыв между ними на
`qwen3:14b`. Сравнительный живой прогон evals по профилям пока не сделан — см.
журнал SPEC-017 и PATCH-017-01.
